...
local PetIllustratedGroupPanel = {
    ...
    toBattleTeamEditor = nil,
}

function PetIllustratedGroupPanel:refreshUI(illustratedGroup)
...
function PetIllustratedGroupPanel:SetToBattleTeamState(callback)
    self.toBattleTeamEditor = callback
end
...
function PetIllustratedGroupPanel:RefreshItem(data, cloneTr, parent)
    ...
    if pet and pet.up == 0 then
        petItemInfo:RefreshExist(self.toBattleTeamEditor)
    else
