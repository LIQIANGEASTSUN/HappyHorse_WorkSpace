...
function PetItemInfo:RefreshAdd(addOnClick)
    ...
    local go = self:Show(PetItemInfoType.Add)
    if not Runtime.CSValid(go) then
        return
    end
    local btn_add = go.transform:Find("btn_add").gameObject:GetComponent(typeof(Button))

    if Runtime.CSValid(btn_add) then
        Util.UGUI_AddButtonListener(btn_add, function()
...
function PetItemInfo:RefreshExist(existOnClick)
    ...
    local btn_exist = find_component(go, 'btn_exist', Button)

    if Runtime.CSValid(btn_exist) then
        Util.UGUI_AddButtonListener(btn_exist, function()
    ...
    --local maxLevel = PetTemplateTool:GetMaxLevel(self.pet.type)
    --local value = self.pet.level / maxLevel
    --slider.value = value
...
function PetItemInfo:RefreshLock(lockOnClick)
    ...
    local go = self:Show(PetItemInfoType.Lock)
    if not Runtime.CSValid(go) then
        return
    end

    local btn_lock = find_component(go, 'btn_lock', Button)
    if Runtime.CSValid(btn_lock) then
        Util.UGUI_AddButtonListener(btn_lock, function()
