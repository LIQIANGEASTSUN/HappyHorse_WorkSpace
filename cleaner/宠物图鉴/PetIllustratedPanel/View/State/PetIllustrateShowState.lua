...
function PetIllustrateShowState:refreshIllustratedGroup()
    local illustrate = self.panel:GetIllustrated()
    local contentTr = illustrate.gameObject.transform:Find("ScrollView/Viewport/Content")
    local illustratedType = contentTr:Find("IllustratedType")

    self.illustratedGroup = { typeParent = contentTr, illustratedType = illustratedType}
    PetIllustratedGroupPanel:SetToBattleTeamState(function(pet) self:ToBattleTeamState(pet) end)
